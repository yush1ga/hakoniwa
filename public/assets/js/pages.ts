...
class ClassAutoFilter {
  ...
  SelectFilter(elmSelect :HTMLElement){
	...
	var elmTbody = <HTMLElement>elmTable.getElementsByTagName("tbody").item(0);
	...
	private _rewriteFilter(elmTrFilter, arrayCols){
		...
		for (let col_i = 0, arrayColsLen :number = arrayCols.length; col_i < arrayColsLen; col_i++) {
			...
			let elmSelect_new = (elmSelect != null)? elmSelect[col_i].cloneNode(false): this._createElement({ element: "select"})
			elmSelect_new.on('change', (v)=>{this.SelectFilter(v)})
			let strSelect = null
			let elmOption = this._createElement({
				element: "option",
			...
			if (elmSelect == null) {
				...
				strSelect = this._getSelectValue(elmSelect[col_i]);
...
function Button_DispFilter(elmInput, strId) {
	...
	const strMes = "お使いのブラウザではオートフィルタを表示できません"
	this.boolExec = false
...
class SortTable{
	private msg_alert :string = "お使いのブラウザではソート機能を利用できません"
	...
	/**
	 * ソートボタン
	 * @param {[type]} stdIdTable  [対象<table>のid]
	 * @param {[type]} arrayColumn [ソート条件を優先順に格納した配列]
	 */
	buttonSort(stdIdTable, arrayColumn){
		let boolExec = false
		let objInterval = setInterval(()=>{
			...
			(()=>{
				if(boolExec) return;
				!boolExec
				this._sortTable(stdIdTable, arrayColumn)
			})
		}, 200);
	}
	...
	private _sortTable(strIdTable :any, arrayColumn :any){
