class MapDevelopment {
    ...
    private command; // = [$set_com];
    private listCommand; // = [$set_listcom];

    private islName; // = [$set_island];
    private listShip; // = [$set_ships];
    private listSatellite; // = [$set_eisei];
    ...
    constructor() {
        for(let i = 0; i < command.length; i++) {
            for(let s = 0; s < $com_count; s++) {
                let comlist2 = listCommand[s];
